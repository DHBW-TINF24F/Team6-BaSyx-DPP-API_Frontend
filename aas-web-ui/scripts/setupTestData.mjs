...
const BACKEND_BASE_URL =
  process.argv[2] ||
  process.env.DPP_BACKEND_BASE_URL ||
  "http://localhost:8080";
...
const TECHNICAL_DATA_SEMANTIC_ID =
  "https://admin-shell.io/idta/TechnicalData/1/0";
const CARBON_FOOTPRINT_SEMANTIC_ID =
...
async function createTestDpp() {
  ...
  if (!result.ok) {
    console.error(
      `[ERROR] Failed to create DPP (${result.status}):`,
      result.data,
    );
    return false;
  }
...
async function registerDpp() {
  ...
  if (!result.ok) {
    console.error(
      `[ERROR] Failed to register DPP (${result.status}):`,
      result.data,
    );
    return false;
  }
...
async function verifyDppByProductId() {
  ...
  const result = await fetchJson(
    `/dppsByProductId/${encodeURIComponent(TEST_PRODUCT_ID)}`,
  );

  if (!result.ok) {
    console.error(
      `[ERROR] Failed to retrieve DPP by Product ID (${result.status}):`,
      result.data,
    );
    return false;
  }
...
async function verifyDppById() {
  ...
  if (!result.ok) {
    console.error(
      `[ERROR] Failed to retrieve DPP by ID (${result.status}):`,
      result.data,
    );
    return false;
  }
