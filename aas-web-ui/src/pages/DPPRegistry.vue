        ...
        <v-dialog v-model="createDialog" max-width="420">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="primary" @click="createDpp">
                        Create
                    </v-btn>
                </v-card-actions>
            </v-card>
...
async function fetchAAS() {
    ...
    aasList.value = aasArray.map((item: any) => ({
        ...
        productId:
            item.id ||
            item.assetInformation?.specificAssetIds?.[0]?.value ||
            '',
        globalAssetId:
