    ...
    <v-container fluid class="pa-4 dpp-detail-container">
        ...
        <template v-else-if="!dpp">
            <div class="text-center mt-12">
                ...
                <p class="text-h6 text-grey mt-4">No DPP found from Shell "{{ shellId }}".</p>
                <p v-if="errorMessage" class="text-body-2 text-grey mt-2">{{ errorMessage }}</p>
                <v-alert v-if="createDppError" type="error" variant="tonal" class="mt-4 text-left" density="compact">
                    {{ createDppError }}
                </v-alert>
                <v-alert
                                        ...
                                        <v-list-item
                                            ...
                                            <template #prepend>
                                                <div
                                                    ...
                                                    :class="{
                                                        'version-index-badge--active': version.dppId === dpp.dppId,
                                                    }">
                                                    {{ vIdx + 1 }}
                                                </div>
                                            </template>
                                            ...
                                            <template #append>
                                                <v-icon v-if="version.dppId === dpp.dppId" size="18" color="primary"
                        ...
                        <div class="d-flex align-center ga-1 mb-1">
                            ...
                            <v-tooltip :text="copiedKey === 'productId' ? 'Copied!' : 'Copy'" location="top">
                                <template #activator="{ props: tt }">
                                    <v-btn
                                        v-bind="tt"
                                        :icon="copiedKey === 'productId' ? 'mdi-check' : 'mdi-content-copy'"
                                        size="x-small"
                                        variant="text"
                                        :color="copiedKey === 'productId' ? 'success' : 'grey'"
                                        @click="copyToClipboard(decodeBase64(dpp.productId), 'productId')" />
                                </template>
                            </v-tooltip>
                        </div>
                        ...
                        <div class="d-flex align-center ga-1 mb-1">
                            ...
                            <v-tooltip :text="copiedKey === 'dppId' ? 'Copied!' : 'Copy'" location="top">
                                <template #activator="{ props: tt }">
                                    <v-btn
                                        v-bind="tt"
                                        :icon="copiedKey === 'dppId' ? 'mdi-check' : 'mdi-content-copy'"
                                        size="x-small"
                                        variant="text"
                    ...
                    <template v-if="namePlateBanner.street !== '-' || namePlateBanner.city !== '-'">
                        ...
                        <div class="meta-value">
                            ...
                            <span
                                ...
                                }}{{ namePlateBanner.city !== '-' ? namePlateBanner.city : '' }}</span
                            >
...
<script lang="ts" setup>
    ...
    function normalizeEntries(raw: unknown, fallbackId?: string): SubmodelEntry[] {
        ...
        if (Array.isArray(r)) {
            for (let i = 0; i < r.length; i++) {
                ...
                res.push({
                    idShort: (obj.idShort as string) || (obj.id as string) || `${fallbackId ?? 'obj'}_${i}`,
                    value: children,
                });
            }
            return res;
    ...
    function renderEntries(entries: SubmodelEntry[], depth: number): VNode[] {
        return entries.map((entry, idx) => {
            ...
            const children = getChildEntries(entry);
            return children.length > 0 ? renderCollection(entry, idx, depth, isLast) : renderLeaf(entry, idx, isLast);
        });
    }
        ...
        setup(props) {
            ...
            return () => {
                ...
                const critRaw = childEntries(
                    findByIdShort(compChildren.length ? compChildren : e, 'CriticalRawMaterials')
                );
                                      ...
                                      h(
                                          ...
                                          critRaw.map((cr) => {
                                              ...
                                              return h('div', { class: 'mc-crm-item' }, [
                                                  ...
                                                  h(
                                                      'div',
                                                      { class: 'mc-crm-val', class2: val === 0 ? 'mc-crm-zero' : '' },
                                                      `${val}%`
                                                  ),
                                              ]);
                                          })
                ...
                return h('div', { class: 'cf-root' }, [
                    ...
                    h('div', { class: 'cf-summary' }, [
                        ...
                        ref !== '-'
                            ? h(
                                  'a',
                                  { href: ref, target: '_blank', rel: 'noopener noreferrer', class: 'cf-ref-link' },
    ...
    const CircularityView = defineComponent({
        ...
        setup(props) {
            ...
            return () => {
                ...
                const recyclePct =
                    parseFloat(flatVal(childEntries(findByIdShort(cir, 'Recyclability')), 'RecyclabilityRate')) || 0;
                    ...
                    h('div', { class: 'circ-layout' }, [
                        ...
                        h('div', { class: 'circ-gauge-wrap' }, [
                            ...
                            h(
                                ...
                                recyclePct >= 90 ? '🟢 Excellent' : recyclePct >= 70 ? '🟡 Good' : '🔴 Low'
                            ),
                        ]),
                        ...
                        h('div', { class: 'circ-right' }, [
                            ...
                            dismantlingFiles.length > 0
                                ? h('div', { class: 'circ-dism-section' }, [
                                      h('div', { class: 'circ-section-title' }, [
                                          h('i', { class: 'mdi mdi-hammer-screwdriver circ-title-icon' }),
                        ...
                        ? h('div', { class: 'pc-temp-section' }, [
                              ...
                              h('div', { class: 'pc-temp-stats' }, [
                                  h('span', { class: 'pc-temp-stat' }, [
                                      h('i', { class: 'mdi mdi-arrow-down-thin pc-temp-cold-icon' }),
                                      ` Min: ${minTemp}°C`,
                                  ]),
                                  h('span', { class: 'pc-temp-stat' }, [
                                      h('i', { class: 'mdi mdi-approximately-equal pc-temp-avg-icon' }),
                                      ` Ø: ${avgTemp}°C`,
                                  ]),
                                  h('span', { class: 'pc-temp-stat' }, [
    ...
    async function createDpp(): Promise<void> {
        ...
        try {
            ...
            const response = await fetch('https://srv01.noah-becker.de/uni/swe/api/dpp/dpps', {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                createDppError.value = errData?.message?.[0]?.text || `Request failed with status ${response.status}.`;
                return;
            }
            ...
            createDppError.value = error instanceof Error ? error.message : 'DPP creation failed.';
        } finally {
    ...
    async function loadDppByDppId(dppId: string): Promise<void> {
        ...
        try {
            ...
            if (!resolvedDpp) {
                dpp.value = null;
                errorMessage.value =
                    data.message?.[0]?.text || data.result?.message?.[0]?.text || 'DPP could not be loaded.';
                return;
            }
...
══════════════════════════════════════════════════════════════════════════ */
    ...
    .np-header {
        ...
        gap: 16px;
        padding: 16px 20px;
        background: linear-gradient(135deg, rgba(var(--v-theme-primary), 0.1), rgba(var(--v-theme-primary), 0.04));
        border-radius: 10px;
        border: 1px solid rgba(var(--v-theme-primary), 0.15);
    }
    .np-header-icon {
