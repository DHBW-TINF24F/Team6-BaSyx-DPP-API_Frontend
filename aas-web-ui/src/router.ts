...
import DPPRegistry from "@/pages/DPPRegistry.vue";
...
function findRouteByName(
  ...
  for (const record of records) {
    ...
    if (record.children && record.children.length > 0) {
      const nestedRecord = findRouteByName(
        record.children as Array<RouteRecordRaw>,
        name,
      );
      if (nestedRecord) {
        return nestedRecord;
...
function findRouteByNameCaseInsensitive(
  records: Array<RouteRecordRaw>,
  name: string,
): RouteRecordRaw | undefined {
  ...
  for (const record of records) {
    ...
    if (record.children && record.children.length > 0) {
      const nestedRecord = findRouteByNameCaseInsensitive(
        record.children as Array<RouteRecordRaw>,
        name,
      );
      if (nestedRecord) {
...
const staticRoutes: Array<RouteRecordRaw> = [
  ...
  },
  {
    ...
    meta: {
      name: "AAS Submodel Viewer",
      subtitle: "Visualize Submodels of AAS",
    },
  },
  {
    path: "/smviewer",
  ...
  },
  {
    ...
    meta: {
      name: "DPP Registry",
      subtitle: "Registry of Digital Product Passports",
    },
  },
  {
...
async function generateModuleRoutes(): Promise<Array<RouteRecordRaw>> {
  ...
  const moduleFolderIndexFileRecords = import.meta.glob(
    "@/pages/modules/*/index.vue",
  );
  const moduleManifestFileRecords = import.meta.glob(
    "@/pages/modules/*.routes.ts",
    { eager: true },
  );
  const moduleFolderManifestFileRecords = import.meta.glob(
    "@/pages/modules/*/routes.ts",
    { eager: true },
  );
  ...
  for (const path in moduleFileRecords) {
    const moduleName =
      path.split("/").pop()?.replace(".vue", "") || "UnnamedModule";
    const manifestPath = path.replace(/\.vue$/, ".routes.ts");
    const moduleManifestRecord = moduleManifestFileRecords[manifestPath] as {
      default?: ModuleRouteManifest;
    };
  ...
  for (const path in moduleFolderIndexFileRecords) {
    ...
    const moduleManifestRecord = moduleFolderManifestFileRecords[
      manifestPath
    ] as { default?: ModuleRouteManifest };

    moduleDefinitionsByName.set(moduleName, {
      moduleName,
      moduleLoader: moduleFolderIndexFileRecords[
        path
      ] as () => Promise<unknown>,
      moduleManifest: moduleManifestRecord?.default,
    });
  ...
  for (const moduleDefinition of moduleDefinitionsByName.values()) {
    ...
    const moduleComponent =
      (await moduleDefinition.moduleLoader()) as ModuleComponentExport;
    ...
    if (
      moduleComponent.default?.moduleTitle &&
      moduleComponent.default?.moduleTitle !== ""
    ) {
    ...
    const isOnlyVisibleWithSelectedAas =
      moduleComponent.default?.isOnlyVisibleWithSelectedAas ?? false;
    const isOnlyVisibleWithSelectedNode =
      moduleComponent.default?.isOnlyVisibleWithSelectedNode ?? false;
    const visibleOnRoutes = moduleComponent.default?.visibleOnRoutes ?? [];
    let preserveRouteQuery =
      moduleComponent.default?.preserveRouteQuery ?? false;
    ...
    const moduleChildren = buildValidatedModuleChildRoutes(
      moduleName,
      routePath,
      parentMeta,
      moduleManifest,
...
export async function createAppRouter(): Promise<Router> {
  ...
  // Data
  const routesStayOnPages: Set<RouteRecordNameGeneric> = new Set([
    "About",
    "NotFound404",
  ]);
  ...
  const routesOnlyMobile: Array<RouteRecordNameGeneric> =
    routesForMobile.filter((x) => !routesForDesktop.includes(x));
  const routesOnlyDesktop: Array<RouteRecordNameGeneric> =
  ...
  const routesToVisualization: Set<RouteRecordNameGeneric> = new Set([
    "ComponentVisualization",
  ]);

  const possibleGloBalAssetIdQueryParameter = [
    "globalAssetId",
    "globalassetid",
  ];
  ...
  const resolveStartRouteName = (query?: Record<string, unknown>): string => {
    ...
    const desired =
      configured && configured.trim() !== "" ? configured.trim() : "AASViewer";
    ...
    if (
      (record.name === "SMViewer" || record.name === "SMEditor") &&
      !envStore.getSmViewerEditor
    ) {
      return "AASViewer";
    }
    ...
    if (
      meta.isOnlyVisibleWithSelectedAas &&
      (!query || !Object.hasOwn(query, "aas") || !String(query.aas).trim())
    ) {
      return "AASViewer";
    }
    if (
  ...
  const saveUrlQueryForSameRoute = (to: any, from: any): void => {
    if (
      Object.hasOwn(from, "name") &&
      Object.hasOwn(to, "name") &&
      from.name === to.name &&
      from.query !== to.query &&
      ...
      if (routesUsingAasOrPathUrlQuery.has(to.name)) {
        if (
          !Object.hasOwn(queryToDispatch, "aas") &&
          Object.hasOwn(queryLoaded, "aas")
        ) {
          queryToDispatch.aas = queryLoaded.aas;
        }
        if (
          !Object.hasOwn(queryToDispatch, "path") &&
  ...
  const saveQueryFromPreviousRoute = (from: any): void => {
    ...
    if (
      routesUsingAasOrPathUrlQuery.has(from.name) ||
      from.path.startsWith("/modules/")
    ) {
      if (
        !Object.hasOwn(queryToDispatch, "aas") &&
        Object.hasOwn(queryLoaded, "aas")
  ...
  const restoreQueryForNewRoute = (
    to: any,
  ): { path: string; name: any; query: LocationQueryRaw } | null => {
    ...
    if (
      routesUsingAasOrPathUrlQuery.has(to.name) ||
      ...
      if (
        routesUsingOnlyAasUrlQuery.has(updatedRoute.name) &&
        Object.hasOwn(updatedRoute.query, "path")
      ) {
        delete updatedRoute.query.path;
      }
      ...
      if (
        routesUsingOnlyPathUrlQuery.has(updatedRoute.name) &&
        Object.hasOwn(updatedRoute.query, "aas")
      ) {
  ...
  const saveAndRestoreUrlQueryOnRouteSwitch = (
    to: any,
    from: any,
  ): { path: string; name: any; query: LocationQueryRaw } | null => {
    if (
  ...
  const removeInvalidQueryParamsForRoute = (
    to: any,
    ...
    if (
      routesUsingOnlyAasUrlQuery.has(to.name) &&
      Object.hasOwn(to.query, "path")
    ) {
      const query = { ...to.query };
      delete query.path;
  ...
  const handleSingleAasAndDeviceRouting = (
    to: any,
  ): { name: string; query?: any } | false | null => {
    if (
    ...
    if (
      ...
      if (["SMViewer", "SMEditor"].includes(to.name as string)) {
        ...
        if (!smViewerEditor.value) {
          return {
            name: (to.name as string).replace("SM", "AAS"),
            query: to.query,
          };
        }
      }
  ...
  const validateAasPathCombination = async (
    to: any,
  ): Promise<{ path: string; query: LocationQueryRaw } | null> => {
    if (
      Object.hasOwn(to.query, "aas") &&
      (to.query.aas as string).trim() !== "" &&
      Object.hasOwn(to.query, "path") &&
  ...
  const fetchAndValidateAasForRoute = async (
    to: any,
    from: any,
  ): Promise<{ path: string; query: LocationQueryRaw } | null> => {
    if (
  ...
  const fetchAndValidateSmeForRoute = async (
    ...
    if (
      ...
      (!aasStore.getSelectedNode ||
        ...
        (Object.hasOwn(from.query, "path") &&
          (from.query.path as string).trim() !==
            (to.query.path as string).trim()))
    ) {
  ...
  const cleanupPluginQueryParams = (
    ...
    if (pathChanged || aasChanged) {
      ...
      const { filteredQuery, removedParams } =
        navigationStore.filterQueryParams(to.query, selectedNode);
  ...
  const handleAasAndSmeDataLoading = async (
    to: any,
    from: any,
  ): Promise<{ path: string; query: LocationQueryRaw } | null> => {
    ...
    const { fetchedSme, redirect } = await fetchAndValidateSmeForRoute(
      to,
      from,
    );
    if (redirect) {
  ...
  const handleOAuthCallback = async (
    to: any,
  ): Promise<{ name: string; replace: true } | null> => {
    ...
    try {
      const { exchangeOAuth2AuthorizationCode } =
        await import("@/composables/Auth/OAuth2Auth");
      ...
      if (!infrastructure || !infrastructure.auth?.oauth2) {
        throw new Error(
          `Infrastructure with ID '${state}' not found or missing OAuth2 config`,
        );
      }
      ...
      if (!issuer) {
        throw new Error(
          "OAuth2 issuer URL not found in callback or infrastructure config",
        );
      }
      ...
      try {
        ...
        if (!["http:", "https:"].includes(issuerUrl.protocol)) {
          throw new Error(
            `Invalid issuer URL protocol: ${issuerUrl.protocol}. Must be http: or https:`,
          );
        }
      } catch (error) {
        if (error instanceof TypeError) {
          throw new Error(
            `Invalid issuer URL format: ${issuer}. Must be a valid HTTP(S) URL.`,
            {
              cause: error,
            },
          );
      ...
      try {
        ...
        console.warn(
          "[OAuth2 Callback] Failed to fetch .well-known configuration, using fallback",
          error,
        );
      }
      ...
      infraStore.setAuthenticationStatusForInfrastructure(
        infrastructure.id,
        true,
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : "OAuth2 authentication failed";
  ...
  router.beforeEach(async function (to, from) {
    ...
    if (to.path === "/" && from.matched.length === 0) {
      const startRouteName = resolveStartRouteName(
